    ...
    [{
        ...
        if (_respawnTime >= 0) then {
            ...
            if (_respawnCounter == 0) then {
                ...
                [_entity, typeOf _entity, vehicleVarName _entity, getPosASL _entity, getDir _entity, _respawnCondition] call FUNC(performVehicleRespawn);
            } else {
...
addMissionEventHandler ["EntityKilled", {
    ...
    if (_respawnTime >= 0) then {
        ...
        private _respawnPosition = _killedEntity getVariable [QGVAR(respawnPosition), getPosASL _killedEntity];
...
[GVAR(AbandonedVehiclesSM), "checkVehicles", {
    ...
    if (!isNull _vehicle) then {
        private _respawnPosition = _vehicle getVariable [QGVAR(respawnPosition), getPosASL _vehicle];
