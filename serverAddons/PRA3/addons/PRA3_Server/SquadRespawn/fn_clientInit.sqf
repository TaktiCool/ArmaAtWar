...
[{
...

    {
        if (!(_x call EFUNC(Common,isAlive)) && !(simulationEnabled _x) && !(isObjectHidden _x)) then {
            _x call EFUNC(Common,removePoint);
        };
        nil
...

    {
        if (_x getVariable [QGVAR(hasRespawnPointAttached), false]) then {
            GVAR(oldSpawns) pushBack ["Player " + name _x, getPos _x, playerSide, -1, "ui\media\fob_ca.paa", "ui\media\fob_ca.paa", []] call EFUNC(Common,addPoint);
        };
        nil
