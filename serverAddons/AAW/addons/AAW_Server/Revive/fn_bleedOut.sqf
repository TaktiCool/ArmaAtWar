...
["unconsciousnessChanged", {
    ...
    if (_state && {GVAR(bleedoutTimer) < 0}) then {
        ...
        GVAR(bleedoutTimer) = [{
            ...
            if ((CLib_Player getVariable [QGVAR(reviveAction), ""]) == "") then {
                ...
                _bloodLevel = _bloodLevel - (CLib_Player getVariable [QGVAR(bleedingRate), 1]) / ([QGVAR(Settings_unconsciousDuration), 500] call CFUNC(getSettingOld));
    ...
    if (!_state && {alive CLib_Player && {GVAR(bloodRefreshTimer) < 0}}) then {
        GVAR(bloodRefreshTimer) = [{
            ...
            _bloodLevel = _bloodLevel + 1 / ([QGVAR(Settings_bloodRestoreDuration), 300] call CFUNC(getSettingOld));
            _bleedingRate = _bleedingRate - _bleedingRate / ([QGVAR(Settings_bloodRestoreDuration), 300] call CFUNC(getSettingOld));
