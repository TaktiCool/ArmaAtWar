...
//_args params ["_crateType", ["_content", []], "_clearOnSpawn", "_displayName", "_constructionPoints"];
_args params ["_cfg"];
...
private _constructionCost = [_cfg + "/constructionCost", 0] call CFUNC(getSetting);
private _supplyType = [_cfg + "/supplyType", ""] call CFUNC(getSetting);
private _supplyCapacity = [_cfg + "/supplyCapacity", 0] call CFUNC(getSetting);
private _constructionType = [_cfg + "/constructionType", ""] call CFUNC(getSetting);
private _constructionCapacity = [_cfg + "/constructionCapacity", 0] call CFUNC(getSetting);
...
private _teamResourcePoints = missionNamespace getVariable [format [QEGVAR(Logistic,constructionPoints_%1), _side], 0];
if (_teamResourcePoints < _constructionCost) exitWith {};
_teamResourcePoints = _teamResourcePoints - _constructionCost;
missionNamespace setVariable [format [QEGVAR(Logistic,constructionPoints_%1), _side], _teamResourcePoints, true];
