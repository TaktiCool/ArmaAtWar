...
["entityCreated", {
            ...
            if !(_selectedMagazine isEqualTo "") then {
                ...
                private _nbrOwnedMagazines = {
                    if (toLower (_x select 0) isEqualTo toLower _selectedMagazine) then {
                        ...
                        if !(_x select 2 && _currentAmmo > 0) then {
                ...
                DUMP(_nbrOwnedMagazines);
                private _nbrMagazinesToAdd = _selectedMagazineDesiredCount - _nbrOwnedMagazines;
                private _cost = ceil sqrt (_selectedMagazineAmmoCount * _selectedMagazineAmmoCost);
                for "_k" from 1 to _nbrMagazinesToAdd do {
                    _totalCost = _totalCost + _cost;
    ...
    private _onProgress = {
        ...
        DUMP(_rearmAmountAvailable);
        DUMP(_totalCosts);
        DUMP(_accumulatedCosts);
